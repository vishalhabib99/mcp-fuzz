...
Static analysis can't see any of that. Only running the code can. It also measures two things static analysis structurally can't: how long each tool actually takes to respond ([Latency check](#latency-check)) and how large its response actually is ([Response size check](#response-size-check)).
