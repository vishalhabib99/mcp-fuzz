...
| [`ChromeDevTools/chrome-devtools-mcp`](https://github.com/ChromeDevTools/chrome-devtools-mcp) | 52.6k | TS | Google's official Chrome DevTools server (v1.10.1, run headless and isolated via `npx`). Clean on crash resilience: **100%/A**, 0 crashes and 0 timeouts across 38 bad-input calls on the 8 tools annotated read-only (22 others skipped as not read-only). One real latency flag: `get_console_message` took 436ms against a ~1ms server median (85.7%/C on latency), the same flag the v0.10.1 run found and one that survives the new 100ms floor. Response size 100%/A. |
